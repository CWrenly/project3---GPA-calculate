...
//計算GPA 當input credits異動時
let inputcredit = document.querySelectorAll(".class-credits");
inputcredit.forEach((creadit) => {
  creadit.addEventListener("input", (e) => {
    setGPA();
  });
});
...
addbtn.addEventListener("click", () => {
  ...
  newInput1.setAttribute("placeholder", "class catergory");
  ...
  newInput3.setAttribute("oninput", "if(value==e)value='';");
  newInput3.setAttribute("oninput", "if(value>10)value=10;if(value<1)value=0");
...
allTrash.forEach((trash) => {
  let tform = trash.parentElement.parentElement;
  console.log(trash.parentElement.parentElement);
  tform.addEventListener("transitionend", (e) => {
    tform.remove();
    setGPA();
  });
});
